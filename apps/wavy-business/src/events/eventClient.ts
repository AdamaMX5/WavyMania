...
// (see TicketService.md), so like wavy-app's ticketClient.ts this is
// configurable. In dev it defaults to /api/ticket, proxied by Vite to a local
// TicketService instance (see vite.config.ts) — TicketService itself sends no
// CORS headers by design (CORS is handled at the NGINX layer in production).
